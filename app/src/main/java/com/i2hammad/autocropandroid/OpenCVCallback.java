    ...
    public void onManagerConnected(int status) {
        switch (status) {
            case LoaderCallbackInterface.SUCCESS: {
                SmartCropper.buildImageDetector(mContext);
...
//                Timber.e("Package installation failed, there is no market app.");
                showMessage("Unable to open Google Market", "The signature scanner depends on the OpenCV Manager application available on Google Play.  We were unable to launch the Google Play Market on your device.");
                break;
            }
        ...
        switch (operation) {
            case InstallCallbackInterface.NEW_INSTALLATION: {
                /*new MaterialDialog.Builder(mContext)
                        ...
                        .content("The signature scanner feature relies on OpenCV.  In order to continue, you must download OpenCV from Google Play Market.  Would you like to download it now?")
                        .cancelable(false)
                        .positiveText("Yes")
                        .negativeText("No")
